...
class pose_estimation:
    ...
    def __init__(self, framerate=75, plotting=False, aruco_dict=cv2.aruco.DICT_ARUCO_MIP_36H12, LMcriteria=(cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_COUNT, 20, 1.19209e-07), ransac=True, ransacTreshold=8):
        # OpenCV Stuff
        ...
        self.criteria_refineLM = LMcriteria  # Default count = 20, epsilon = 1.19209e-07
        self.ransac = ransac
        self.ransacTreshold = ransacTreshold
    ...
    def estimate_pose_board(self,reference_board:cv2.aruco.Board, target_board:cv2.aruco.Board, corners, ids, cameraMatrix, distCoeffs=np.array([[0., 0., 0., 0., 0.]])):
        """
        Arguments:
        -----------
        ...
        if ids is not None and len(ids) > 0:
            ...
            # Set Solving Method
            solve_flag = cv2.SOLVEPNP_SQPNP #cv2.SOLVEPNP_EPNP
            # if self.prev_target_tvec is not None:
            #     # Estimate pose of Reference board
            #     ref_val, ref_rvec, ref_tvec = cv2.solvePnP(ref_obj_pts,ref_img_pts,cameraMatrix,self.cv_dist_coeffs,
            #                                 rvec=self.prev_ref_rvec,tvec=self.prev_ref_tvec,useExtrinsicGuess=True,flags=solve_flag)
                
            #     # Estimate Pose of Target Board
            #     target_val, target_rvec, target_tvec = cv2.solvePnP(target_obj_pts,target_img_pts,cameraMatrix,self.cv_dist_coeffs,
            ...
            if self.ransac:
                # Pose of Reference Board
                ref_val, ref_rvec, ref_tvec, ref_inliers = cv2.solvePnPRansac(ref_obj_pts,ref_img_pts,cameraMatrix,distCoeffs,
                                                            rvec=self.prev_ref_rvec,tvec=self.prev_ref_tvec,useExtrinsicGuess=False,flags=solve_flag, iterationsCount = 1000, reprojectionError=self.ransacTreshold)
                ref_obj_pts, ref_img_pts = ref_obj_pts[ref_inliers].reshape(-1,1,3), ref_img_pts[ref_inliers].reshape(-1,1,2)
                
                
                # # Pose of Target Board
                target_val, target_rvec, target_tvec, target_inliers = cv2.solvePnPRansac(target_obj_pts,target_img_pts,cameraMatrix,distCoeffs,
                                                                rvec=self.prev_target_rvec,tvec=self.prev_target_tvec,useExtrinsicGuess=False,flags=solve_flag, iterationsCount = 1000, reprojectionError=self.ransacTreshold)
                target_obj_pts, target_img_pts = target_obj_pts[target_inliers].reshape(-1,1,3), target_img_pts[target_inliers].reshape(-1,1,2)
                
            # Pose of Reference Board
            ref_val, ref_rvec, ref_tvec = cv2.solvePnP(ref_obj_pts,ref_img_pts,cameraMatrix,distCoeffs,
                                            rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag)
            # Pose of Target Board
            target_val, target_rvec, target_tvec = cv2.solvePnP(target_obj_pts,target_img_pts,cameraMatrix,distCoeffs,
                                                    rvec=None,tvec=None,useExtrinsicGuess=False,flags=solve_flag)
            ...
            ref_rvec, ref_tvec = cv2.solvePnPRefineLM(ref_obj_pts,ref_img_pts,cameraMatrix,distCoeffs,
                                                      ref_rvec,ref_tvec, self.criteria_refineLM)
            
            target_rvec, target_tvec = cv2.solvePnPRefineLM(target_obj_pts,target_img_pts,cameraMatrix,distCoeffs,
                                                            target_rvec,target_tvec, self.criteria_refineLM)
            ...
            if self.prev_target_tvec is not None:
                ...
                self.ref_tvel, self.ref_rvel, self.target_tvel, self.target_rvel = 0, 0, 0, 0
            ...
            if target_val != 0 and ref_val != 0:
                ...
                np.fill_diagonal(covariance,std_dev) # Not a correct measurement of covariance, to remove as noise modelling is taken care of by Q & R initialization
                ...
                # Relative pose via homogeneous transformations
                # cam_ref is camera pose relative to reference model space
                # cam_ref = np.block([[ref_rot_mat, ref_tvec], [0, 0, 0, 1]])
                # target_cam = np.block([[target_rot_mat.T, -target_rot_mat.T @ target_tvec], [0, 0, 0, 1]])
                # target_ref = (target_cam @ cam_ref).T
