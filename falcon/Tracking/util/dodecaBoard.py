...
if __name__ == "__main__":
    ...
    target_offset = (-0.42030256, -1.35511477, 211.53434921)
    
    ref_marker_size = 35  # dodecahedron edge length in mm
    ref_pentagon_size = 40
    
    targetPoints = generate(target_marker_size, target_pentagon_size, target_offset)
    refPoints = generate(ref_marker_size, ref_pentagon_size)
    
    fileTime = f'{time.strftime("%Y-%m-%d_%H_%M_%S")}'
    np.savetxt(f'../../Calibration/calib_files/tool/dodecaBoard_target_{fileTime}.txt', targetPoints.reshape(11,12))
    np.savetxt(f'../../Calibration/calib_files/tool/dodecaBoard_ref_{fileTime}.txt', refPoints.reshape(11,12))